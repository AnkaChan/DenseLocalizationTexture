...
class UnetLocalizer(object):
    ...
    def optimizer_initializer(self):
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate, ).minimize(self.cost, global_step=self.optStep)
    ...
    def predict(self, imgs, batchSize = 10):
        ...
        for iBatch in range(numBatch):
            fdVal = {self.x: imgs[iBatch*batchSize:(iBatch+1)*batchSize, ...],
                     self.keep_prob: 1.,
                     }

            output = self.sess.run(self.logits, feed_dict=fdVal)
